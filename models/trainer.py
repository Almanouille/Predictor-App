...
class FootballModelTrainer:
    ...
    def prepare_training_data(self, leagues: List[int] = None, seasons: List[int] = None,
        ...
        for league_id in leagues:
            for season in seasons:
                try:
                    ...
                    features_df, targets = self.feature_calculator.compute_training_features(
        ...
        logger.info(f'Total matches nb={len(X)}')
        ...
        logger.info(f"  Features number: {X.shape[1]}")
        logger.info(f"  Training samples: {len(X_train)}")
        logger.info(f"  Test samples: {len(X_test)}")
        logger.info(f"  Class distribution: {np.bincount(y)}")

        # Feature selection
        best_k, best_features, best_score = self.get_best_k(X=X, y=y, k_values=[10, 15, 20, 25, 30, 35, 40, 45, 50, 55])
    ...
    def train_single_model(self, model_name: str, X_train: np.ndarray, y_train: np.ndarray,
        ...
        # Calculate metrics
        metrics = self._calculate_metrics(y_train, y_train_pred, y_test, y_test_pred, y_test_proba)
    ...
    def save_best_model(self, version: str = None) -> Optional[str]:
        ...
        return self.save_model(self.best_model_name, version)
    ...
    def evaluate_model(self, model_name: str, X_test: np.ndarray, y_test: np.ndarray) -> Dict[str, Any]:
        ...
        metrics = self._calculate_metrics(y_test, y_pred, y_test, y_pred, y_proba)
    ...
    def _calculate_metrics(self, y_train: np.ndarray, y_train_pred: np.ndarray,
                           ...
                           y_test_proba: np.ndarray) -> Dict[str, float]:
        """
        ...
        :param y_test_proba: Predicted probabilities for test set.
...
def train():
    ...
    results = trainer.train_top_models(X_train, y_train, X_test, y_test)
